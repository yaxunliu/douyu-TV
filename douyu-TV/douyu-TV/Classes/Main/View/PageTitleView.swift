...
class PageTitleView: UIView {
    ...
    fileprivate var previousBtn : UIButton = UIButton()
    fileprivate var scrollLine : UIView = UIView()
    fileprivate var pageTitles:[String]
    fileprivate var titleButtons:[UIButton] = [UIButton]()
    fileprivate var selectColor : (CGFloat ,CGFloat ,CGFloat) = (255.0,155.0,76.0)
    
    // MARK: ------ 对外公开的方法
    func titleViewDidScroll(_ sourceIndex : Int ,targetIndex : Int , progress : CGFloat){
        // 滚动条开始滚动
    ...
    fileprivate func addSubButton(){
        ...
        for (index, title) in pageTitles.enumerated(){
            ...
            btn.addTarget(self, action: #selector(self.titlebtnClick(_:)), for: .touchUpInside)
            if index == 0 {
                previousBtn = btn
    ...
    // MARK: ------ 按钮的点击事件
     @objc fileprivate func titlebtnClick(_ button : UIButton){
        ...
        /// 4.通知代理
        delegate.pageTitleViewDidClick(self, selectedIndex: index)
